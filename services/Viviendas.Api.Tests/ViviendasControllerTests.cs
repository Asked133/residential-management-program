...
public class ViviendasControllerTests : IAsyncLifetime
{
    ...
    public async Task GetViviendas_ReturnsOk_WithMockedData()
    {
        ...
        mockSupabaseService.Setup(s => s.GetViviendasAsync(It.IsAny<Guid>(), It.IsAny<string>(), It.IsAny<Guid>()))
            .ReturnsAsync(new List<ViviendaDto> { new ViviendaDto { Id = Guid.NewGuid(), Direccion = "Test Dir" } });
